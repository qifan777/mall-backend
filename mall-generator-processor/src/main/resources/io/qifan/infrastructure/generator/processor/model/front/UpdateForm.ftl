...
<script lang="ts" setup>
    ...
    const init = async () => {
        ...
        updateForm.value = {
            ...await api.${uncapitalizeTypeName}Controller.findById({id: updateForm.value.id || ''})
        }
    }
    watch(
    ...
    const handleConfirm = () => {
        ...
        updateFormRef.value?.validate(
            assertFormValidate(() => {
                api.${uncapitalizeTypeName}Controller.save({body: updateForm.value}).then(async (res) => {
